...
class SudokuGame:
    ...
    def create_puzzle(self, difficulty):
        ...
        self.solution = [row[:] for row in self.sudoku]  # Store the solution for validation
    ...
    def create_grid(self):
        ...
        validate_button = tk.Button(self.root, text="Validate", command=self.check_solution)
        validate_button.grid(row=9, column=0, columnspan=9)
    ...
    def validate_entry(self, event, row, col):
        ...
        if entry_value.isdigit() and 1 <= int(entry_value) <= 9:
            ...
            self.entries[row][col].configure(bg='white')  # Reset background color
        else:
            self.sudoku[row][col] = 0
            self.entries[row][col].configure(bg='red')  # Set background color to red

        if self.sudoku[row][col] == self.solution[row][col]:
            self.entries[row][col].configure(bg='green')  # Set background color to green for correct entries
            ...
            for j in range(9):
                ...
                if self.sudoku[i][j] == 0:
                    if entry.get().isdigit() and int(entry.get()) == self.solution[i][j]:
                        entry.configure(bg='green')  # Set background color to green for correct entries
                    else:
                        entry.configure(bg='red')  # Set background color to red for incorrect entries
        if self.validate_solution():
            for i in range(9):
                for j in range(9):
                    entry = self.entries[i][j]
                    entry.configure(bg='green')  # Set background color to green for all correct entries
            # You can add your logic for handling a correct solution here.
            print("Congratulations! You solved the puzzle.")
        else:
            # You can add your logic for handling an incorrect solution here.
            pass
